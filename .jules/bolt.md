...
## 2026-03-23 - [Consolidating Multiple Counts with $facet]
**Learning:** Performing multiple independent `count_documents` calls on the same collection (e.g., for dashboard statistics or filtered badge counts) creates unnecessary database roundtrips and network latency. These can be consolidated into a single query using MongoDB's `$facet` aggregation stage, where each facet contains its own `$match` and `$count`.
**Action:** Use `$facet` for services that require multiple counts from a single collection. Always provide default fallbacks for aggregation results to handle empty collections gracefully.
