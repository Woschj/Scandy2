...
def _get_dev_secret_key():
    """Returns a persistent secret key for development to avoid session invalidation on hot-reload."""
    key_file = Path(__file__).resolve().parents[2] / '.dev_secret_key'
    if key_file.exists():
        try:
