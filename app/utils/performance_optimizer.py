...
class QueryOptimizer:
    ...
    def get_dashboard_statistics_optimized() -> Dict[str, Any]:
        ...
        try:
            # 1. Tool-Statistiken via Aggregation
            tool_pipeline = [
                {'$match': {'deleted': {'$ne': True}}},
                {
                    '$group': {
                        ...
                        'available': {
                            '$sum': {
                                '$cond': [
                                    {'$in': ['$status', ['available', 'verfügbar', 'Available', 'bereit', 'Bereit']]},
                                    1, 0
                                ]
                            }
                        },
            ...
            # 2. Consumable-Statistiken via Aggregation
            cons_pipeline = [
                {'$match': {'deleted': {'$ne': True}}},
                {
                    '$group': {
