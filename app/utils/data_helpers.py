            ...
            try:
                ...
                if isinstance(group_id, str) and len(group_id) == 24:
                    try:
                        ...
                        logger.warning(f"Fehler bei ObjectId-Konvertierung für group_id {group_id}: {str(e)}")
                        query_id = group_id
                ...
                logger.warning(f"Fehler bei Gruppenverarbeitung für group_id {group_id}: {str(e)}")
                group_names.append(str(group_id))
        
        return ', '.join(group_names)
    except Exception as e:
        logger.error(f"Fehler bei Gruppenverarbeitung: {str(e)}")
...
def format_software_list(software_list):
    ...
    try:
        ...
        logger.warning(f"Fehler bei Software-Listen-Formatierung: {str(e)}")
        return ''
...
def format_boolean_field(value, true_text="Ja", false_text="Nein"):
    ...
    try:
        ...
        logger.warning(f"Fehler bei Boolean-Feld-Formatierung: {str(e)}")
        return ''