...
def convert_id_for_query(id_value: str) -> Union[str, ObjectId]:
    ...
    try:
        ...
        logger.warning(f"Fehler bei ID-Behandlung: {str(e)}")
...
def find_document_by_id(collection: str, id_value: str, mongodb_instance=None):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"ObjectId-Konvertierung fehlgeschlagen: {str(e)}")
...
def find_user_by_id(user_id: str, mongodb_instance=None):
    ...
    try:
        ...
        logger.error(f"Fehler beim Laden des Users mit ID {user_id}: {str(e)}")
        return None
