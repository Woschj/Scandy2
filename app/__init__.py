...
def _configure_app(app, test_config):
    """Lädt die Konfiguration und setzt Systemnamen."""
    from app.config import config
    import os, secrets
...
def _init_logging(app):
    """Initialisiert Logger und Verzeichnisse."""
...
def _init_mongodb_and_ids(app):
    """Initialisiert MongoDB-Verbindung und normalisiert IDs."""
    import logging, os
    try:
...
def _cleanup_old_sessions(app):
    ...
    try:
        ...
        session_lifetime = app.config.get('PERMANENT_SESSION_LIFETIME', datetime.timedelta(days=7))
        ...
        remaining_files = os.listdir(session_dir)[:100]
...
def _init_session(app):
    """Konfiguriert Session-Management."""
    import os, datetime
    ...
    app.config.setdefault('PERMANENT_SESSION_LIFETIME', datetime.timedelta(days=7))
...
def _register_request_hooks(app):
    """Registriert Before/After-Request-Hooks für Sessions."""
...
def _start_background_tasks(app):
    """Starte asynchrone Hintergrund-Tasks."""
...
def _register_routes(app):
    """Registriert Routen auf App-Level."""
...
def _register_context_processors_and_filters(app):
    """Registriert Template-Context-Processors und Custom-Filters."""
    from flask import g
    from app.utils.context_processors import register_context_processors
    from app.utils.filters import status_color, priority_color
...
def create_app(test_config=None):
    ...
    # 2. Logging und Verzeichnisse
    _init_logging(app)
    register_filters(app)
    ensure_directories_exist()

    # 3. MongoDB-Initialisierung
    _init_mongodb_and_ids(app)

    # 4. Login-Manager und Sessions
    login_manager.init_app(app)
    _init_session(app)
    _register_request_hooks(app)

    # 5. Blueprints und Context-Prozessoren
    _register_context_processors_and_filters(app)
    ...
    # 7. Routen und Fehlerbehandlung
    _register_routes(app)
    handle_errors(app)
    register_filters(app)

    # 8. Weitere Initialisierungen
    Compress(app)
