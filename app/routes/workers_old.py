...
# Import der zentralen find_document_by_id Funktion
...
def index():
    """Zeigt die Mitarbeitende-Übersicht an"""
    try:
        # Hole alle nicht gelöschten Mitarbeitende der aktuellen Abteilung
        from flask import g
        ...
        # Für jeden Mitarbeitende die aktiven Ausleihen zählen und Nutzende-Informationen hinzufügen
        for worker in workers:
            ...
            # Hole Nutzende-Informationen falls vorhanden
        ...
        logger.error(f"Fehler beim Laden der Mitarbeitende: {str(e)}", exc_info=True)
        flash('Fehler beim Laden der Mitarbeitende', 'error')
...
def add():
    ...
    if request.method == 'POST':
        ...
        if not current_dept:
            flash('Bitte Abteilung wählen, bevor Sie einen Mitarbeitende anlegen', 'error')
        ...
        try:
            ...
            # Wenn Barcode eindeutig ist, füge den Mitarbeitende hinzu
            worker_data = {
                'barcode': barcode,
                'firstname': firstname,
                'lastname': lastname,
            ...
            flash('Mitarbeitende erfolgreich hinzugefügt', 'success')
...
def details(original_barcode):
    """Details eines Mitarbeitendes anzeigen und bearbeiten"""
    try:
        departments = get_departments_from_settings()
        ...
        if request.method == 'POST':
            ...
            # Prüfen, ob der Mitarbeitende existiert
            worker = mongodb.find_one('workers', {'barcode': original_barcode, 'deleted': {'$ne': True}})
            if not worker:
                flash('Mitarbeitende nicht gefunden.', 'error')
                return redirect(url_for('workers.index'))
            ...
            flash('Mitarbeitende erfolgreich aktualisiert', 'success')
        ...
        # Hole Verbrauchsmaterial-Ausgaben des Mitarbeitendes
        ...
        logger.error(f"Fehler beim Laden der Mitarbeitendedetails: {str(e)}", exc_info=True)
        flash('Fehler beim Laden der Mitarbeitendedetails', 'error')
        return redirect(url_for('workers.index'))
...
def edit(barcode):
    """Bearbeitet einen Mitarbeitende über Modal"""
    try:
        firstname = request.form.get('firstname')
        lastname = request.form.get('lastname')
        ...
        return jsonify({
            'success': True, 
            'message': 'Mitarbeitende erfolgreich aktualisiert',
            'redirect': url_for('workers.details', original_barcode=new_barcode)
        })
        
    except Exception as e:
        logger.error(f"Fehler beim Aktualisieren des Mitarbeitendes: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'message': 'Fehler beim Aktualisieren des Mitarbeitendes'}), 500
...
def delete_by_barcode(barcode):
    ...
    try:
        # Prüfe ob der Mitarbeitende existiert
        worker = mongodb.find_one('workers', {'barcode': barcode, 'deleted': {'$ne': True}})
        ...
        # Prüfe ob der Mitarbeitende noch Werkzeuge ausgeliehen hat
        ...
        return jsonify({
            'success': True,
            'message': 'Mitarbeitende erfolgreich gelöscht'
        })
        
    except Exception as e:
        logger.error(f"Fehler beim Löschen des Mitarbeitendes: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
...
@login_required
def timesheet_list():
    """Zeigt die Wochenberichte für den aktuellen Nutzende an (für alle Rollen mit timesheet_enabled)"""
    ...
    # Prüfe ob Wochenbericht-Feature für den Nutzende aktiviert ist
...
def teilnehmer_timesheet_list():
    """Spezielle Route für Teilnehmende zu den Wochenberichten"""
