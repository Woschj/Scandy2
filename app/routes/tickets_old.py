...
# Verwendet die zentrale convert_id_for_query Funktion aus app.utils.id_helpers

# Verwendet die zentrale normalize_id_for_database Funktion aus app.utils.id_helpers

# Verwendet die zentrale find_document_by_id Funktion aus app.utils.id_helpers
...
def create():
    ...
    try:
        ...
        # Hole Handlungsfelder des Nutzendes (für alle Rollen außer Admin)
        user_handlungsfelder = []
        if current_user.role != 'admin':
            # Hole Handlungsfelder aus der Nutzende-Konfiguration
            user_settings = mongodb.find_one('users', {'username': current_user.username})
            if user_settings and user_settings.get('handlungsfelder'):
                user_handlungsfelder = user_settings['handlungsfelder']
                print(f"DEBUG: Nutzende {current_user.username} (Rolle: {current_user.role}) hat Handlungsfelder: {user_handlungsfelder}")
            else:
                print(f"DEBUG: Nutzende {current_user.username} (Rolle: {current_user.role}) hat keine Handlungsfelder zugewiesen")
...
@permission_required('tickets', 'view')
def view(ticket_id):
    """Zeigt die Details eines Tickets für den Nutzende."""
    logging.info(f"Lade Ticket {ticket_id} für Nutzende {current_user.username}")
    ...
    # Prüfe ob der Nutzende berechtigt ist, das Ticket zu sehen
    has_permission = check_ticket_permission(ticket, current_user.username, current_user.role)
    
    if not has_permission:
        logging.error(f"Nutzende {current_user.username} hat keine Berechtigung für Ticket {ticket_id}")
    ...
    try:
        ...
        # Hole alle Nutzende für die Zuweisung (falls benötigt)
...
def add_message(ticket_id):
    ...
    try:
        ...
        logging.info(f"Versuche Nachricht zu speichern: Ticket {ticket_id}, Nutzende {current_user.username}, Nachricht: {message}")
...
def detail(id):
    ...
    try:
        ...
        # Hole alle Nutzende aus der Hauptdatenbank und wandle sie in Dicts um
        users = mongodb.find('users', {'is_active': True})
        users = [dict(user) for user in users]
...
def delete(id):
    ...
    try:
        ...
        # Prüfe Berechtigungen: Nur Admins und Mitarbeitende können Tickets löschen
...
def public_create_order():
    """Interne Auftragserstellung für eingeloggte Nutzende."""
...
def _handle_auftrag_creation(external=False):
    ...
    if request.method == 'POST':
        try:
            ...
            # Für alle Nutzende zur Bestätigungsseite
...
def auftrag_details_page(id):
    try:
        ...
        # Prüfe Berechtigungen: Admins/Mitarbeitende/Teilnehmende können alle Tickets sehen, normale User nur ihre eigenen oder zugewiesenen
        has_permission = check_ticket_permission(ticket, current_user.username, current_user.role)
