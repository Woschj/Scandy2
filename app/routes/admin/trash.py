...
def delete_consumable_permanent(barcode):
    try:
        ...
        logger.error(f"Fehler beim permanenten Löschen des Verbrauchsmaterials: {str(e)}")
        return jsonify({'success': False, 'message': 'Interner Serverfehler'}), 500
...
def delete_worker_permanent(barcode):
    try:
        ...
        logger.error(f"Fehler beim permanenten Löschen des Mitarbeiters: {str(e)}")
        return jsonify({'success': False, 'message': 'Interner Serverfehler'}), 500
...
def delete_logo(filename):
    ...
    try:
        ...
        logger.error(f"Fehler beim Löschen des Logos: {str(e)}")
