...
class ToolService:
    ...
    def get_tool_statistics(self) -> Dict[str, Any]:
        """
        Holt Statistiken für Werkzeuge (optimiert via Aggregation)
        ...
        try:
                        ...
                        'counts': [
                            {
                                '$group': {
                                    ...
                                    'total_tools': {'$sum': 1},
                                    'available_tools': {
                                        '$sum': {'$cond': [{'$and': [{'$eq': ['$status', 'verfügbar']}, {'$eq': ['$is_borrowed', False]}]}, 1, 0]}
                                    },
                                    'borrowed_tools': {
            ...
            result = results[0]
            counts = result['counts'][0] if result['counts'] else {
