...
class ConsumableService:
    ...
    def get_statistics() -> Dict[str, Any]:
        ...
        Reduziert Datenbank-Abfragen und In-Memory-Verarbeitung von O(N) auf O(1).
        """
        try:
            # Basis-Filter für die Aggregation (beachtet die aktuelle Abteilung)
            match_query = {'deleted': {'$ne': True}}
            if getattr(g, 'current_department', None):
                match_query['department'] = g.current_department

            # Aggregation-Pipeline zur Berechnung aller Statistiken in einer Abfrage (Bolt ⚡)
            pipeline = [
                {'$match': match_query},
                {
                    '$facet': {
            ...
            # _id Feld aus stock_levels entfernen
