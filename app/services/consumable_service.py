...
class ConsumableService:
    ...
    def get_statistics() -> Dict[str, Any]:
        ...
        Reduziert Datenbank-Abfragen und Netzwerk-Overhead von O(N) auf O(1).
        """
        try:
            # Basis-Filter für die Abteilung (Department Scoping)
            match_query = {'deleted': {'$ne': True}}
            if getattr(g, 'current_department', None):
                match_query['department'] = g.current_department

            # Aggregation-Pipeline zur Berechnung aller Statistiken in einem Roundtrip
            pipeline = [
                {'$match': match_query},
                {
                    '$facet': {
                        'base_counts': [
                            {
                                '$group': {
                                    ...
                                    'sufficient': {
                                        '$sum': {
                                            '$cond': [{'$gte': ['$quantity', {'$ifNull': ['$min_quantity', 0]}]}, 1, 0]
                                        }
                                    },
                                    'warning': {
                                        '$sum': {
                                            '$cond': [
                                                {
                                                    '$and': [
                                                        {'$lt': ['$quantity', {'$ifNull': ['$min_quantity', 0]}]},
                                                        {'$gt': ['$quantity', 0]}
                                                    ]
                                                },
                                                1, 0
            ...
            result = list(mongodb.aggregate('consumables', pipeline))
            ...
            data = result[0]
            base = data['base_counts'][0] if data.get('base_counts') else {}

            stats = {
                'total_consumables': base.get('total', 0),
                'categories': {c['_id']: c['count'] for c in data.get('categories', [])},
                'locations': {l['_id']: l['count'] for l in data.get('locations', [])},
                'stock_levels': {
