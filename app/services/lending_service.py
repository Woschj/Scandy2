...
class LendingService:
    ...
    def get_active_lendings() -> list:
        ...
        try:
            ...
            active_lendings = mongodb.aggregate('lendings', pipeline)

            # Post-Processing für Namensformatierung
            enriched_lendings = []
            for lending in active_lendings:
                tool = lending.get('tool_info', {})
                worker = lending.get('worker_info', {})

                lending['tool_name'] = tool.get('name', 'Unbekannt')
                lending['worker_name'] = f"{worker.get('firstname', '')} {worker.get('lastname', '')}".strip() or 'Unbekannt'

                # Temporäre Felder entfernen
                lending.pop('tool_info', None)
                lending.pop('worker_info', None)
            ...
            logger.error(f"Fehler beim Laden aktiver Ausleihen: {e}")
            return []
    ...
    def get_recent_consumable_usage(limit: int = 10) -> list:
        ...
        Verschiebt Sortierung und Limitierung in die Datenbank und eliminiert N+1 Lookups.
        """
        try:
            ...
            recent_usages = mongodb.aggregate('consumable_usages', pipeline)
            ...
            logger.error(f"Fehler beim Laden der Verbrauchsmaterial-Entnahmen: {e}")
            return []
    
    @staticmethod
    def get_worker_consumable_history(worker_barcode: str) -> List[Dict[str, Any]]:
        """
        Holt die Verbrauchsmaterial-Historie für einen Mitarbeiter (optimiert via Aggregation) (Bolt ⚡)
        """
        try:
            # Aggregation zur Vermeidung von N+1 Lookups und Python-Sortierung
            pipeline = [
                ...
                {'$unwind': {'path': '$consumable_info', 'preserveNullAndEmptyArrays': True}}
            ]
            ...
            for usage in usages:
                consumable = usage.get('consumable_info', {})
                usage['consumable_name'] = consumable.get('name', 'Unbekannt')
                usage.pop('consumable_info', None)

            return usages
            
        except Exception as e:
            logger.error(f"Fehler beim Laden der Verbrauchsmaterial-Historie: {e}")
            return []
    
    @staticmethod
    def get_current_lending(tool_barcode: str) -> Optional[Dict[str, Any]]:
        """
        Holt die aktuelle Ausleihe für ein Werkzeug (optimiert via Aggregation) (Bolt ⚡)
        """
        try:
            # Aggregation zur Vermeidung mehrerer Datenbank-Roundtrips
            pipeline = [
                {'$match': {'tool_barcode': tool_barcode, 'returned_at': None}},
                {'$limit': 1},
                {
                    '$lookup': {
                        'from': 'workers',
            ...
            result = mongodb.aggregate('lendings', pipeline)
            
            if not result:
                return None
                
            current_lending = result[0]
            worker = current_lending.get('worker_info', {})
            tool = current_lending.get('tool_info', {})

            current_lending['worker_name'] = f"{worker.get('firstname', '')} {worker.get('lastname', '')}".strip() or 'Unbekannt'
            current_lending['tool_name'] = tool.get('name', 'Unbekannt')

            # Temporäre Felder entfernen
            current_lending.pop('worker_info', None)
            current_lending.pop('tool_info', None)
            
            return current_lending
            
        except Exception as e:
            logger.error(f"Fehler beim Laden der aktuellen Ausleihe: {e}")
            return None
    
    @staticmethod
    def get_tool_lending_history(tool_barcode: str) -> List[Dict[str, Any]]:
        """
        Holt die Ausleihhistorie für ein Werkzeug (optimiert via Aggregation) (Bolt ⚡)
        """
        try:
            # Aggregation zur Vermeidung von N+1 Lookups und Python-Sortierung
            pipeline = [
            ...
            lendings = mongodb.aggregate('lendings', pipeline)
            ...
            logger.error(f"Fehler beim Laden der Ausleihhistorie: {e}")
            return []
