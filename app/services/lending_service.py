...
class LendingService:
    ...
    def process_lending_request(
        ...
        try:
            ...
            if item_type not in valid_types:
                return (
                    False,
                    f'Ungültiger Item-Typ. Erlaubt: {", ".join(valid_types)}',
                    {},
                )
            ...
            if item_type == "tool":
                return LendingService._process_tool_lending(
                    item_barcode, worker_barcode, action, worker
                )
            elif item_type == "consumable":
                return LendingService._process_consumable_lending(
                    item_barcode, worker_barcode, action, quantity, worker
                )
            else:
    ...
    @staticmethod
    def _process_tool_lending(
        item_barcode: str, worker_barcode: str, action: str, worker: Dict[str, Any]
    ) -> Tuple[bool, str, Dict[str, Any]]:
        """Verarbeitet Werkzeug-Ausleihe/Rückgabe mit verbesserter Konsistenz"""
        try:
            # Prüfe ob Werkzeug existiert
            tool = mongodb.find_one(
                "tools", {"barcode": item_barcode, "deleted": {"$ne": True}}
            )
            if not tool:
                return False, "Werkzeug nicht gefunden", {}

            if action == "lend":
                return LendingService._lend_tool(
                    item_barcode, worker_barcode, tool, worker
                )
            elif action == "return":
                return LendingService._return_tool(
                    item_barcode, worker_barcode, tool, worker
                )
            else:
    ...
    def _lend_tool(
        ...
        try:
            ...
            active_lending = mongodb.find_one(
                "lendings", {"tool_barcode": item_barcode, "returned_at": None}
            )

            if active_lending:
                current_worker = mongodb.find_one(
                    "workers", {"barcode": active_lending["worker_barcode"]}
                )
                worker_name = (
                    f"{current_worker['firstname']} {current_worker['lastname']}"
                    if current_worker
                    else "Unbekannt"
                )
                return (
                    False,
                    f"Dieses Werkzeug ist bereits an {worker_name} ausgeliehen",
                    {},
                )
            ...
            tool_update_result = mongodb.update_one(
                "tools",
            ...
            logger.info(
                f"Werkzeug {tool['name']} erfolgreich an {worker['firstname']} {worker['lastname']} ausgeliehen"
            )

            return (
                True,
                f'Werkzeug {tool["name"]} wurde an {worker["firstname"]} {worker["lastname"]} ausgeliehen',
                {
                    "tool_name": tool["name"],
    ...
    def _return_tool(
        ...
        try:
            logger.info(
                f"_return_tool aufgerufen: item_barcode={item_barcode}, worker_barcode={worker_barcode}"
            )
            ...
            if worker_barcode and active_lending["worker_barcode"] != worker_barcode:
                ...
                logger.warning(
                    f"Berechtigungsfehler: Werkzeug wurde von {worker_name} ausgeliehen"
                )
            ...
            try:
                ...
                if isinstance(lending_id, str):
                    try:
                        ...
                        logger.error(
                            f"Fehler beim Konvertieren der ID: [Interner Fehler]"
                        )
                        return (
                            False,
                            f"Fehler beim Konvertieren der ID: [Interner Fehler]",
                ...
                if not lending_update_result:
                    logger.error(
                        f"Fehler beim Markieren der Rückgabe für Ausleihe {active_lending['_id']}"
                    )
                ...
                logger.error(
                    f"Exception beim Markieren der Rückgabe: [Interner Fehler]"
                )
                return (
                    False,
                    f"Fehler beim Markieren der Rückgabe: [Interner Fehler]",
                    {},
                )
            ...
            try:
                logger.info(
                    f"Versuche Werkzeug-Status zu aktualisieren: {item_barcode}"
                )
                tool_update_result = mongodb.update_one(
                    "tools",
                    {"barcode": item_barcode},
                ...
                logger.error(
                    f"Exception beim Aktualisieren des Werkzeug-Status: [Interner Fehler]"
                )
                # Rollback versuchen
                try:
                    mongodb.update_one(
                ...
                return (
                    False,
                    f"Fehler beim Aktualisieren des Werkzeug-Status: [Interner Fehler]",
                    {},
                )
            ...
            return (
                True,
                f'Werkzeug {tool["name"]} wurde erfolgreich zurückgegeben',
                {"tool_name": tool["name"], "returned_at": datetime.now()},
            )

        except Exception as e:
            logger.error(
                f"Fehler bei Werkzeug-Rückgabe: [Interner Fehler]", exc_info=True
            )
            return False, f"Fehler bei der Rückgabe: [Interner Fehler]", {}

    @staticmethod
    def _process_consumable_lending(
        item_barcode: str,
        worker_barcode: str,
        action: str,
        quantity: int,
        ...
        try:
            ...
            consumable = mongodb.find_one(
                "consumables", {"barcode": item_barcode, "deleted": {"$ne": True}}
            )
            if not consumable:
            ...
            if current_quantity < quantity:
                return (
                    False,
                    f'Nicht genügend {consumable.get("name", "")} verfügbar (verfügbar: {current_quantity}, benötigt: {quantity})',
                    {},
                )
            ...
            consumable_update_result = mongodb.update_one(
                "consumables",
                {"barcode": item_barcode},
                {
                    "$set": {
            ...
            logger.info(
                f"{quantity}x {consumable.get('name', '')} erfolgreich an {worker.get('firstname', '')} {worker.get('lastname', '')} ausgegeben"
            )
            ...
            return (
                False,
                f"Fehler bei der Verbrauchsmaterial-Verarbeitung: [Interner Fehler]",
                {},
            )
        ...
        try:
            pipeline = [
                ...
                },
                {
                    "$addFields": {
                        "tool_name": {
                            "$ifNull": ["$tool_info.name", "Unbekanntes Werkzeug"]
                        },
                        "worker_name": {
                            "$cond": {
                                "if": {
                                    "$and": [
                                        {"$gt": ["$worker_info.firstname", None]},
                                        {"$gt": ["$worker_info.lastname", None]},
                                    ]
                                },
                                "then": {
                                    "$concat": [
    ...
    @staticmethod
    def get_recent_consumable_usage(
        limit: int = 10, days: int = None, only_outputs: bool = False
    ) -> list:
        """Holt die letzten Verbrauchsmaterial-Entnahmen (Optimiert mit Aggregation Bolt ⚡)"""
        try:
            from datetime import timedelta

            # Filter-Pipeline aufbauen
            match_query = {}
            if days:
                ...
                [
                    ...
                    },
                    {
                        "$addFields": {
                            "consumable_name": {
                                "$ifNull": [
                                    "$consumable_info.name",
                                    "Unbekanntes Verbrauchsmaterial",
                                ]
                            },
                            "worker_name": {
                                "$cond": {
            ...
            logger.error(
                f"Fehler beim Laden der Verbrauchsmaterial-Entnahmen: [Interner Fehler]"
            )
            return []
    ...
    def get_worker_consumable_history(worker_barcode: str) -> List[Dict[str, Any]]:
        """
        Holt die Verbrauchsmaterial-Historie für einen Mitarbeiter (Optimiert mit Aggregation Bolt ⚡)
        ...
        try:
            pipeline = [
                ...
                {
                    "$addFields": {
                        "consumable_name": {"$ifNull": ["$consumable_info.name", ""]},
                        "consumable_barcode": "$consumable_barcode",
                    }
                },
                {"$project": {"consumable_info": 0}},
            ]
            ...
            logger.error(
                f"Fehler beim Laden der Verbrauchsmaterial-Historie: [Interner Fehler]"
            )
            return []
    ...
    def get_current_lending(tool_barcode: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            current_lending = mongodb.find_one(
                "lendings", {"tool_barcode": tool_barcode, "returned_at": None}
            )

            if current_lending:
                # Worker-Informationen hinzufügen
                worker = mongodb.find_one(
                    "workers", {"barcode": current_lending["worker_barcode"]}
                )
                if worker:
                    current_lending["worker_name"] = (
    ...
    def get_tool_lending_history(tool_barcode: str) -> List[Dict[str, Any]]:
        """
        Holt die Ausleihhistorie für ein Werkzeug (Optimiert mit Aggregation Bolt ⚡)
    ...
    def validate_lending_consistency() -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            for tool in tools:
                ...
                active_lending = mongodb.find_one(
                    "lendings", {"tool_barcode": barcode, "returned_at": None}
                )
            ...
            orphaned_lendings = list(
                mongodb.find(
                    "lendings", {"returned_at": None, "tool_barcode": {"$exists": True}}
                )
            )
            ...
            for lending in orphaned_lendings:
                ...
                tool = mongodb.find_one(
                    "tools", {"barcode": tool_barcode, "deleted": {"$ne": True}}
                )
            ...
            message = (
                f"Konsistenzprüfung abgeschlossen: {len(issues)} Probleme gefunden"
                if issues
                else "Alle Daten sind konsistent"
            )
        ...
        try:
            ...
            for tool in tools:
                ...
                if active_lending and status != "ausgeliehen":
                    ...
                    mongodb.update_one(
                        "tools", {"barcode": barcode}, {"$set": {"status": "verfügbar"}}
                    )
                    fixed_count += 1
            ...
            for barcode in set(tool_barcodes):
                ...
                if count > 1:
                    ...
                    duplicate_lendings.sort(
                        key=lambda x: x.get("lent_at", datetime.min), reverse=True
                    )
            ...
            return (
                True,
                f"Inkonsistenzen behoben: {fixed_count} Werkzeug-Status korrigiert, {cleaned_count} Ausleihen bereinigt",
                {
            ...
            return (
                False,
                f"Fehler beim Beheben der Inkonsistenzen: [Interner Fehler]",
                {},
            )

    @staticmethod
    def return_tool_centralized(
        tool_barcode: str, worker_barcode: str = None
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            logger.info(
                f"return_tool_centralized aufgerufen: tool_barcode={tool_barcode}, worker_barcode={worker_barcode}"
            )

            # Finde aktive Ausleihe
            if worker_barcode:
                # Spezifischer Mitarbeiter
                lending = mongodb.find_one(
                    "lendings",
                    {
                        "tool_barcode": tool_barcode,
                        "worker_barcode": worker_barcode,
            ...
            if not lending:
                logger.warning(
                    f"Keine aktive Ausleihe für Werkzeug {tool_barcode} gefunden"
                )
            ...
            worker = mongodb.find_one(
                "workers",
                {"barcode": lending["worker_barcode"], "deleted": {"$ne": True}},
            )
            worker_name = (
                f"{worker['firstname']} {worker['lastname']}" if worker else "Unbekannt"
            )
            logger.info(f"Worker gefunden: {worker_name}")

            # Verwende die zentrale Rückgabe-Funktion
            success, message, _ = LendingService._return_tool(
                tool_barcode, lending["worker_barcode"], tool, worker or {}
            )
            ...
            if success:
                ...
                return (
                    True,
                    f'Werkzeug {tool["name"]} wurde von {worker_name} erfolgreich zurückgegeben',
                )
            else:
            ...
            logger.error(
                f"Fehler bei der zentralen Rückgabe: [Interner Fehler]", exc_info=True
            )
            return False, f"Fehler bei der Rückgabe: [Interner Fehler]"
